...
tagged_columns AS (
    ...
    FROM TABLE(INFORMATION_SCHEMA.TAG_REFERENCES_ALL_COLUMNS(
        'RAW_DEV.RAW_SIS.STUDENT_RAW', 'TABLE'))
    WHERE COLUMN_NAME IS NOT NULL
    GROUP BY OBJECT_DATABASE, OBJECT_SCHEMA, OBJECT_NAME, TAG_NAME
)
SELECT
    tc.FULL_TABLE_NAME,
    tc.TABLE_NAME,
    tc.TOTAL_COLUMNS,
    COALESCE(SUM(tg.TAGGED_COLUMNS), 0) AS TAGGED_COLUMN_COUNT,
    ROUND(100.0 * COALESCE(SUM(tg.TAGGED_COLUMNS), 0) / (tc.TOTAL_COLUMNS * 4), 2) AS COVERAGE_PCT
FROM table_columns tc
LEFT JOIN tagged_columns tg ON tc.FULL_TABLE_NAME = tg.FULL_TABLE_NAME
GROUP BY tc.FULL_TABLE_NAME, tc.TABLE_NAME, tc.TOTAL_COLUMNS
...
SELECT
    ...
    (SELECT COUNT(DISTINCT COLUMN_NAME) 
     ...
     WHERE TAG_NAME = 'FERPA_CATEGORY') AS STUDENT_COLUMNS_TAGGED,
    
    -- Masking policy coverage
    (SELECT COUNT(*) FROM GOVERNANCE.POLICIES.INFORMATION_SCHEMA.MASKING_POLICIES) AS MASKING_POLICIES_DEFINED,
    
    -- Row access policy coverage  
    (SELECT COUNT(*) FROM GOVERNANCE.POLICIES.INFORMATION_SCHEMA.ROW_ACCESS_POLICIES) AS ROW_ACCESS_POLICIES_DEFINED,
